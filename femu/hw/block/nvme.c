...
#define SQ_POLLING_PERIOD_NS	(500000)
#define CQ_POLLING_PERIOD_NS	(500000)

#define FEMU_WHITEBOX_MODE      0
#define FEMU_BLACKBOX_MODE      1
#define FEMU_DEF_NOSSD_MODE		2

#define DEBUG_NVME_SQ
#define DEBUG_NVME_CQ
...
static unsigned int leap_used_memslots;

/* Assign/unassign. Keep an unsorted array of non-overlapping
 * memory regions in n->mem. */
static void leap_unassign_memory(NvmeCtrl *n, uint64_t start_addr, uint64_t size)
{
    int from, to, nregions = n->mem->nregions;
    /* Track overlapping/split regions for sanity checking. */
    int overlap_start = 0, overlap_end = 0, overlap_middle = 0, split = 0;

    for (from = 0, to = 0; from < nregions; ++from, ++to) {
        struct vhost_memory_region *reg = n->mem->regions + to;
        uint64_t reglast;
        uint64_t memlast;
        uint64_t change;
        ...
        /* clone old region */
        if (to != from) {
            memcpy(reg, n->mem->regions + from, sizeof *reg);
        }
        ...
        memcpy(n->mem->regions + nregions, reg, sizeof *reg);
        reg->memory_size = start_addr - reg->guest_phys_addr;
        assert(reg->memory_size);
        change = memlast + 1 - reg->guest_phys_addr;
        reg = n->mem->regions + nregions;
        reg->memory_size -= change;
        assert(reg->memory_size);
        reg->guest_phys_addr += change;
        reg->userspace_addr += change;
        /* Never add more than 1 region */
        assert(n->mem->nregions == nregions);
        ++n->mem->nregions;
        ++split;
    }
}
...
#if 0
static bool leap_can_merge(uint64_t start1, uint64_t size1, uint64_t start2,
        uint64_t size2)
{
    ram_addr_t offset;
    int mfd, rfd;
...
static void leap_assign_memory(NvmeCtrl *n, uint64_t start_addr, uint64_t size,
        uint64_t uaddr)
{
    int from, to;
    struct vhost_memory_region *merged = NULL;
    for (from = 0, to = 0; from < n->mem->nregions; ++from, ++to) {
        struct vhost_memory_region *reg = n->mem->regions + to;
    ...
    assert(to <= n->mem->nregions + 1);
    n->mem->nregions = to;
}

static struct vhost_memory_region *leap_find_reg(NvmeCtrl *n,
        uint64_t start_addr, uint64_t size)
{
    int i, nregions = n->mem->nregions;
    for (i = 0; i < nregions; i++) {
        struct vhost_memory_region *reg = n->mem->regions + i;
        if (ranges_overlap(reg->guest_phys_addr, reg->memory_size,
                    start_addr, size)) {
            return reg;
        }
...
static bool leap_cmp_memory(NvmeCtrl *n, uint64_t start_addr, uint64_t size,
        uint64_t uaddr)
{
    struct vhost_memory_region *reg = leap_find_reg(n, start_addr, size);
    uint64_t reglast;
    uint64_t memlast;
...
static void leap_set_memory(MemoryListener *listener,
        MemoryRegionSection *section, bool add)
{
    NvmeCtrl *n = container_of(listener, NvmeCtrl, memory_listener);
    hwaddr start_addr = section->offset_within_address_space;
    ram_addr_t size = int128_get64(section->size);
    bool log_dirty = memory_region_get_dirty_log_mask(section->mr) & \
                     ~(1 << DIRTY_MEMORY_MIGRATION);
    int s = offsetof(struct vhost_memory, regions) + \
            (n->mem->nregions + 1) * sizeof n->mem->regions[0];
    RAMBlock *rb = section->mr->ram_block;
    size_t pgsz = qemu_ram_pagesize(rb);
    void *ram;

    printf("Coperd,rb=%p,s=%d,pgsz=0x%lx\n", rb, s, pgsz);
    n->mem = g_realloc(n->mem, s);
...
#if 0
    uint64_t alignage;
    /* Now align the start down to a page boundary */
    alignage = ((uintptr_t)ram) & (pgsz - 1);
    if (alignage) {
        printf("Coperd,%s,bummer,guest memory region not aligned\n", __func__);
        ram -= alignage;
        size += alignage;
        start_addr -= alignage;
    }
    /* Now align the size up to a page boundary */
    alignage = size & (pgsz - 1);
    if (alignage) {
        size += pgsz - alignage;
    }
#endif
    ...
    if (add) {
        if (!leap_cmp_memory(n, start_addr, size, (uintptr_t)ram)) {
            /* Region exists with same address. Nothing to do. */
            return;
        }
    } else {
        if (!leap_find_reg(n, start_addr, size)) {
            /* Removing region that we don't access. Nothing to do. */
            return;
        }
    }

    leap_unassign_memory(n, start_addr, size);
    if (add) {
        /* Add given mapping, merging adjacent regions if any */
        leap_assign_memory(n, start_addr, size, (uintptr_t)ram);
    } else {
        /* Remove old mapping for this memory, if any. */
        leap_unassign_memory(n, start_addr, size);
    }
    n->mem_changed_start_addr = MIN(n->mem_changed_start_addr, start_addr);
    n->mem_changed_end_addr = MAX(n->mem_changed_end_addr, start_addr + size - 1);
    n->memory_changed = true;
    leap_used_memslots = n->mem->nregions;
}

static bool leap_section(MemoryRegionSection *section)
{
    return memory_region_is_ram(section->mr) &&
        !memory_region_is_rom(section->mr);
}

static void leap_region_add(MemoryListener *listener,
        MemoryRegionSection *section)
{
    NvmeCtrl *n = container_of(listener, NvmeCtrl, memory_listener);

    if (!leap_section(section)) {
        return;
    }

    n->n_mem_sections++;
    n->mem_sections = g_renew(MemoryRegionSection, n->mem_sections,
            n->n_mem_sections);
    n->mem_sections[n->n_mem_sections - 1] = *section;
    memory_region_ref(section->mr);
    leap_set_memory(listener, section, true);
}

static int leap_nvme_init(NvmeCtrl *n)
{
    n->memory_listener = (MemoryListener) {
        .region_add = leap_region_add,
        .priority = 10
    };

    n->mem = g_malloc0(offsetof(struct vhost_memory, regions));
    n->n_mem_sections = 0;
    n->mem_sections = NULL;

    memory_listener_register(&n->memory_listener, &address_space_memory);

    printf("Coperd,LEAP NVMe memory listener registered !!! \n");

    return 0;
}

/* most non-init callers ignore the error */
static int leap_write(NvmeCtrl *n, LeapMsg *msg, int *fds, int fd_num)
{
    CharBackend *chr = &n->chardev;
    int ret, size = LEAP_HDR_SIZE + msg->size;

    /* Coperd: TODO: in case of MR changes, we need to do remap */

    if (qemu_chr_fe_set_msgfds(chr, fds, fd_num) < 0) {
        error_report("Coperd,Failed to set msg fds.");
        return -1;
    }

    ret = qemu_chr_fe_write_all(chr, (const uint8_t *)msg, size);
    if (ret != size) {
        error_report("Failed to write msg. Wrote %d instead of %d.", ret, size);
        return -1;
    }
...
/* Coperd: make sure memory regions are sorted based on start_addr */
static inline int mrcmp(const void *a, const void *b)
{
    return ((struct vhost_memory_region *)a)->guest_phys_addr >
        ((struct vhost_memory_region *)b)->guest_phys_addr;
}

static void leap_sort_mem_regions(NvmeCtrl *n)
{
    qsort(n->mem->regions, n->mem->nregions, sizeof(struct vhost_memory_region),
            mrcmp);
}

static int leap_set_mem_table(NvmeCtrl *n)
{
    int fds[LEAP_MEMORY_MAX_NREGIONS];
    int i, fd;
    size_t fd_num = 0;

    LeapMsg msg = {
        .request = LEAP_SET_MEM_TABLE,
        .flags = 0x1,
    };

    leap_sort_mem_regions(n);

    printf("Coperd,===%s===,n->mem->nregions=%d\n", __func__, n->mem->nregions);
    for (i = 0; i < n->mem->nregions; i++) {
        struct vhost_memory_region *reg = n->mem->regions + i;
        ram_addr_t offset;
        MemoryRegion *mr;
        ...
        if (fd > 0) {
            ...
            assert(fd_num < LEAP_MEMORY_MAX_NREGIONS);
            printf("\n\nCoperd,%s,region[%lu],userspace_addr:0x%llx,memory_size:0x%llx,"
                    "guest_phys_addr:0x%llx,mmap_offset:0x%lx,fd:%d\n\n",
                    __func__, fd_num, reg->userspace_addr, reg->memory_size,
                    reg->guest_phys_addr, offset, fd);
            fds[fd_num++] = fd;
        } else {
            printf("Coperd,%s,%d,ERROR memfile fd < 0\n", __func__, __LINE__);
        }
    }
    ...
    if (!fd_num) {
        error_report("Coperd,Leap,Failed initializing vhost-user memory map, "
    ...
    msg.size += fd_num * sizeof(LeapMemoryRegion);

    if (leap_write(n, &msg, fds, fd_num) < 0) {
        return -1;
    }
...
{
    return sqid < (n->num_queues + 1) && n->sq[sqid] != NULL ? 0 : -1;
}

static int nvme_check_cqid(NvmeCtrl *n, uint16_t cqid)
{
    return cqid < (n->num_queues + 1) && n->cq[cqid] != NULL ? 0 : -1;
}
...
static void nvme_update_cq_head(NvmeCQueue *cq)
{
    ...
    if (cq->db_addr) {
        ...
        if (n->leap_qp_mapping && cq->head != cq->prev_head) {
#ifdef DEBUG_NVME_CQ
            printf("Coperd,%s,cq:%d,head=%"PRIu32"\n", __func__, cq->cqid, cq->head);
#endif
            cq->prev_head = cq->head;
        }
    }
}
...
static void nvme_post_cqes_io(void *opaque)
{
    ...
    ntt = (ntt == 0) ? qemu_clock_get_ns(QEMU_CLOCK_REALTIME) + \
          CQ_POLLING_PERIOD_NS : ntt;
    timer_mod(cq->timer, ntt);

    /* Coperd: only interrupt guest when we "do" complete some I/Os */
    if (nr_processed_cqes > 0 && n->ioq_isr_enabled) {
        nvme_isr_notify(cq);
    }
}

#ifndef USE_PREV_IMPL
static void nvme_post_cqes(void *opaque)
{
    nvme_post_cqes_io(opaque);
}
#else
static void nvme_post_cqes(void *opaque)
{
...
static int nvme_snapshot_read(NvmeCtrl *n, NvmeCmd *cmd)
{
    //NvmeRwCmd *rw = (NvmeRwCmd *)cmd;
    // lba
    // version number
...
static int nvme_snapshot_write(NvmeCtrl *n, NvmeCmd *cmd)
{
    //NvmeRwCmd *rw = (NvmeRwCmd *)cmd;
    // lba *
    // num_lbas
...
static uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_SNAPSHOT_READ:
        return nvme_snapshot_read(n, cmd);
    case NVME_SNAPSHOT_WRITE:
        return nvme_snapshot_write(n, cmd);
    ...
    int contig)
{
    ...
    if (sqid == 0) {
        ...
        if (sq->timer) {
            printf("Coperd,%s,WARNING,SQ[%d] timer not empty! You're trying to"
                    "create a new one w/o destroying previous one correctly!\n",
                    __func__, sqid);
            exit(EXIT_FAILURE);
        }
    ...
    /* Coperd: kick start the repeative timer for I/O queues */
    if (sqid) {
...
static uint16_t nvme_create_sq(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    /* Coperd: register QP for this SQ */
    leap_qpbuf_register_sq(sq);
...
static int nvme_vector_unmask(PCIDevice *dev, unsigned vector, MSIMessage msg)
{
    ...
    for (qid = 1; qid <= n->num_queues; qid++) {
        cq = n->cq[qid];
        if (!cq) {
            continue;
        }
...
static uint16_t nvme_create_cq(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    /* disable prev structure created by BIOS */
    if (n->cq[cqid] != NULL) {
    ...
    if (nvme_init_cq(cq, n, prp1, cqid, vector, qsize + 1,
        ...
        error_report("nvme_create_cq failed\n");
        g_free(cq);
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ...
    /* Coperd: register QP for this CQ */
    leap_qpbuf_register_cq(cq);
...
static uint16_t nvme_get_feature(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_NUMBER_OF_QUEUES:
        /* Coperd: 0 based value for # of IO queues */
        req->cqe.n.result =
...
static uint64_t ns_blks(NvmeNamespace *ns, uint8_t lba_idx)
{
    ...
    for (i = 0; i < n->nlbaf; i++) {
        printf("Coperd,%s,lbaf[%d].ds=%d\n", __func__, i, id_ns->lbaf[i].ds);
    }
...
static uint16_t nvme_doorbell_buffer_config(NvmeCtrl *n, const NvmeCmd *cmd)
{
    ...
    /* Coperd: register Shadow Doorbell buffer to WPT */
    leap_qpbuf_register_db(n);

    /* This assumes all I/O queues are created before this command is handled.
     * We skip the admin queues. */
    for (i = 1; i < n->num_queues + 1; i++) {
        ...
        if (sq) {
            ...
            printf("Coperd,DBBUF,sq[%d]:db_addr=%" PRIu64 ","
                    "eventidx_addr=%" PRIu64 "\n", i,
                    sq->db_addr, sq->eventidx_addr);
        }
        if (cq) {
            ...
            printf("Coperd,DBBUF,cq[%d]:db_addr=%" PRIu64 ","
                    "eventidx_addr=%" PRIu64 "\n", i,
                    cq->db_addr, cq->eventidx_addr);
        }
    }
...
static uint16_t nvme_admin_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    switch (cmd->opcode) {
    case 0xf7:
        n->ioq_processing_enabled = !n->ioq_processing_enabled;
        if (n->ioq_processing_enabled == false) {
            printf("Coperd, ***** Disable IO NVMe Queue Processing! ***\n");
        } else {
            printf("Coperd, ***** Enable IO NVMe Queue Processing! ***\n");
        }
        return NVME_SUCCESS;
    case 0xf8:
        n->ioq_isr_enabled = !n->ioq_isr_enabled;
        if (n->ioq_isr_enabled == false) {
            printf("Coperd, ***** Disable IO NVMe Queue interrupts! ***\n");
        } else {
            printf("Coperd, ***** Enable IO NVMe Queue interrupts! ***\n");
        }
        return NVME_SUCCESS;
    case NVME_ADM_CMD_GPA_MAP:
        gfn = le32_to_cpu(cmd->cdw10);
        gpa = gfn << PAGE_SHIFT;
        len = PAGE_SIZE;

        printf("Coperd,GFN (cdw10) = %lx, gpa=%lx\n", gfn, gpa);

        void *user_buf = cpu_physical_memory_map(gpa, &len, 0);
        if (!user_buf) {
            printf("Coperd,cpu_physical_memory_map failed\n");
            return NVME_ACCESS_DENIED;
        }
        printf("Coperd,guest[%d] buf addr:%p,hpa:%"PRIu64",%s\n", getpid(),
                user_buf, leap_hva2hfn(user_buf), (char *)user_buf);

        getchar();

        len = 4096;
        cpu_physical_memory_unmap(user_buf, len, 0, len);

        return 0;

    case NVME_ADM_CMD_GPA_UNMAP:
        return NVME_SUCCESS;

    case NVME_ADM_CMD_WPT_UNMAP:
        gfn = le32_to_cpu(cmd->cdw10);
        gpa = gfn << PAGE_SHIFT;
        ret = leap_do_qp_unmapping(gbuf_hva);
        if (ret == -1)
            return NVME_ACCESS_DENIED;
        return NVME_SUCCESS;

    case NVME_ADM_CMD_WPT_MAP:
        gfn = le32_to_cpu(cmd->cdw10);
        gpa = gfn << PAGE_SHIFT;
        ret = leap_do_qp_mapping(gpa);
        if (ret == -1)
            return NVME_ACCESS_DENIED;
        return NVME_SUCCESS;
...
static void nvme_update_sq_tail(NvmeSQueue *sq)
{
    ...
    if (sq->db_addr) {
        ...
        if (n->leap_qp_mapping && sq->tail != sq->prev_tail) {
#ifdef DEBUG_NVME_SQ
            printf("Coperd,%s,sq:%d,tail=%"PRIu32"\n", __func__, sq->sqid, sq->tail);
#endif
            sq->prev_tail = sq->tail;
        }
    }
}
...
static void nvme_process_sq_admin(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list)) &&
        ...
        if (status != NVME_NO_COMPLETE) {
            ...
            printf("Coperd,admin cmd(0x%x) status:%d\n", cmd.opcode, status);
        }
    }
...
static void nvme_process_sq_io(void *opaque)
{
    ...
    uint16_t status;
    hwaddr addr;
    NvmeCmd cmd, *qpcmd = NULL;
    NvmeRequest *req;
    int processed = 0;

    /* Coperd: for Quantum Leap, we skip IO processing in the hypervisor */
    if (!n->ioq_processing_enabled) {
        //printf("Coperd,%s,WARNING,QEMU NVMe I/O path disabled!\n", __func__);
        return;
    }
...
#ifdef DEBUG_NVME_SQ
        /* Coperd: ensure that we can map QP in QEMU AS correctly */
        qpcmd = leap_qpbuf_get_sqe(n, sq->sqid, sq->head);
        if (!qpcmd || leap_nvme_cmd_cmp(&cmd, qpcmd)) {
            printf("Coperd,Leap:failed to get SQE from SQ[%d] HVA\n", sq->sqid);
            exit(EXIT_FAILURE);
        } else {
            printf("Coperd,SQ[%d,%d],", sq->sqid, sq->head);
            leap_print_nvme_cmd(qpcmd);
        }
#endif
...
static void nvme_clear_ctrl(NvmeCtrl *n, bool shutdown)
{
    ...
    if (shutdown) {
        printf("FEMU Shutdown NVMe Controller ...");
    } else {
        printf("QEMU Disable NVMe Controller ...");
    }

    /* Coperd: do we need to register vNVME-shutdown to WPT? */

    if (shutdown) {
        nvme_clear_guest_notifier(n);
    }

    for (i = 0; i < n->num_queues + 1; i++) {
        if (n->sq[i] != NULL) {
...
static int nvme_start_ctrl(NvmeCtrl *n)
{
    ...
    printf("FEMU Start NVMe Controller ...");

    /* Coperd: TODO: can we do this sooner? */
    if (qemu_chr_fe_backend_connected(&n->chardev)) {
        leap_set_mem_table(n);
    }
    ...
    unsigned size)
{
    ...
    case 0x24:
        ...
        printf("Coperd,n->bar.aqa=%d\n", NVME_AQA_ASQS(n->bar.aqa));
        sleep(1);
        break;
    case 0x28:
        n->bar.asq = data;
...
#ifdef DEBUG_NVME_CQ
        printf("Coperd,%s,Now CQ[%d].head=%d\n", __func__, qid, new_val);
#endif
...
#ifdef DEBUG_NVME_SQ
        printf("Coperd,%s,Now SQ[%d].tail=%d\n", __func__, qid, new_val);
#endif
...
static int nvme_init(PCIDevice *pci_dev)
{
    ...
    /* Coperd: HACK for Leap, fixed to physical SSD size; TODO */
#if 1
    bs_size = 960197124096;
    bs_size = 274877906944; // 256 GB
    //bs_size = 549755813888; // 512 GB
    //bs_size = 2199023255552; // 2TB
    //bs_size = 1099511627776; // 1TB
    //bs_size = 2190433320960; // OCSSD-size
#endif
    ...
    if (!n->heap_storage) {
        printf("Coperd,FEMU:failed to malloc [%"PRIu64"] bytes for backend"
                "storage, please make sure you have enough memory\n", bs_size);
        exit(EXIT_FAILURE);
    }
#endif
    ...
    n->sq = g_malloc0(sizeof(*n->sq) * (n->num_queues + 1));
    n->cq = g_malloc0(sizeof(*n->cq) * (n->num_queues + 1));
    n->namespaces = g_malloc0(sizeof(*n->namespaces) * n->num_namespaces);
    n->elpes = g_malloc0((n->elpe + 1) * sizeof(*n->elpes));
    n->aer_reqs = g_malloc0((n->aerl + 1) * sizeof(*n->aer_reqs));
    n->features.int_vector_config = g_malloc0((n->num_queues + 1) *
            sizeof(*n->features.int_vector_config));
    n->ioq_isr_enabled = true;
    //n->ioq_processing_enabled = false;
    ...
    ret = leap_qpbuf_init(n);
    if (ret) {
        printf("Coperd,leap_qpbuf_init failed,ret=%d\n", ret);
        exit(EXIT_FAILURE);
    }
...
static Property nvme_props[] = {
    ...
    DEFINE_PROP_UINT32("queues", NvmeCtrl, num_queues, 2), //stnovako
    //DEFINE_PROP_UINT32("queues", NvmeCtrl, num_queues, 1),
    ...
    DEFINE_PROP_UINT8("mdts", NvmeCtrl, mdts, 5/*10*/),
    ...
    DEFINE_PROP_UINT8("lba_index", NvmeCtrl, lba_index, 0/*3*/),
    ...
    DEFINE_PROP_UINT16("oncs", NvmeCtrl, oncs, 0/*NVME_ONCS_DSM*/),
    DEFINE_PROP_UINT16("vid", NvmeCtrl, vid, 0x1d1d),
    DEFINE_PROP_UINT16("did", NvmeCtrl, did, 0x2807), /*0x1f1f for QEMU*/
    ...
    DEFINE_PROP_UINT8("leap_qp_mapping", NvmeCtrl, leap_qp_mapping, 1),
    DEFINE_PROP_BOOL("enable_ioq", NvmeCtrl, ioq_processing_enabled, 0),
