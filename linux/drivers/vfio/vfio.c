...
struct iommu_group *vfio_iommu_group_get(struct device *dev)
{
	...
	if (group || !noiommu || iommu_present(dev->bus)) {
		printk("Coperd,%s,NORMAL iommu mode,noiommu=%d\n", __func__, noiommu);
		return group;
	}
	...
	printk("Coperd,%s,set group name to vfio-noiommu\n", __func__);
	...
	printk("Coperd, Adding kernel taint for vfio-noiommu group on device\n");
#endif
...
static void *vfio_noiommu_open(unsigned long arg)
{
	...
	if (arg != VFIO_NOIOMMU_IOMMU) {
		printk("Coperd,%s, not passing VFIO_NOIOMMU_IOMMU!\n", __func__);
		return ERR_PTR(-EINVAL);
	}
	if (!capable(CAP_SYS_RAWIO)) {
		printk("Coperd,%s, not capable of CAP_SYS_RAWIO!\n", __func__);
			       ...
			       unsigned int cmd, unsigned long arg)
{
	if (cmd == VFIO_CHECK_EXTENSION) {
		printk("Coperd,%s,ioctl(VFIO_CHECK_EXTENSION), noiommu=%d, (arg==VFIO_NOIOMMU_IOMMU)=%d, ret=%d\n", __func__, noiommu, (arg == VFIO_NOIOMMU_IOMMU), noiommu && (arg == VFIO_NOIOMMU_IOMMU) ? 1 : 0);
...
static int vfio_group_set_container(struct vfio_group *group, int container_fd)
{
	...
	if (group->noiommu && !capable(CAP_SYS_RAWIO)) {
		printk("Coperd,%s,Error NOIOMMU:noiommu=%d,CAP_SYS_RAWIO=%d\n",
		       __func__, group->noiommu, capable(CAP_SYS_RAWIO));
		return -EPERM;
	}
...
static int vfio_group_get_device_fd(struct vfio_group *group, char *buf)
{
	...
	if (group->noiommu) {
		printk("Coperd,%s,vfio-noiommu device opened by user!\n", __func__);
