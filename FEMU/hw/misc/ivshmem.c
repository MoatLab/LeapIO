...
typedef struct IVShmemState {
    ...
    CharBackend chr; /* for connecting DBVM to get their QEMU memory file map */
    LeapMemory memory;
    int fds[LEAP_MEMORY_MAX_NREGIONS];
    int num_fds;
    LeapMsg read_msg;
    size_t read_msg_size;
    LeapMemTableRegion mem_table[LEAP_MEMORY_MAX_NREGIONS];
    ...
    MemoryRegion pqp_doorbell;  /* Coperd: BAR 1 (physical NVMe doorbell) */
    MemoryRegion qpair_bar;     /* Coperd: BAR 4 for queue pairs */
    MemoryRegion bar3;          /* Coperd: BAR 3 for QEMU memory file map */
    MemoryRegion server_bar2;   /* used with server_chr */
    MemoryRegion host_mem_region;
    MemoryRegion dmr_mem_region;
    MemoryRegion dmabuf_mem_region;
    /* Coperd: Leap buf for vQP, vQP-DB and pQP */
    void *shbuf;
    /* Coperd: Leap buf for pQP-DB */
    void *doorbell_buf;
    int doorbell_bufsz; /* Coperd: in bytes */
    uint8_t do_qpair_mapping;
    uint8_t do_dmabuf_mapping;
...
static void leap_map_dmabuf_mr(IVShmemState *s)
{
    int i;
    void *dmr_buf;
    int idx = 0;
    struct huge_1g_memory_region *hmr = get_hmr();

    if (!posix_memalign(&dmr_buf, 4096, 4096)) {
        exit(EXIT_FAILURE);
    }
    assert(dmr_buf);
    memset(dmr_buf, 0, 4096);

    for (i = 0; i < 16; i++) {
        if (hmr[i].start_hva == 0) {
            break;
        }

        ((uint64_t *)dmr_buf)[idx] = hmr[i].start_hva;
        idx++;
        ((uint64_t *)dmr_buf)[idx] = hmr[i].start_hpa;
        idx++;
        ((uint64_t *)dmr_buf)[idx] = hmr[i].size;
        idx++;
    }

    memory_region_init_ram_ptr(&s->dmr_mem_region, OBJECT(s),
            "leap-dmabuf-desc-mem-region", 4096, dmr_buf);
    //s->dmr_mem_region.should_skip = true;
    /* Coperd: the offset to be 16MB, immediately after qpairs */
    memory_region_add_subregion(&s->bar3, 16ULL << 20, &s->dmr_mem_region);


    /* Map hugepage to BAR3 again */
    memory_region_init_ram_ptr(&s->dmabuf_mem_region, OBJECT(s),
            "leap-dmabuf-mem-region", (1ULL << 30), (void *)hmr[0].start_hva);
    memory_region_add_subregion(&s->bar3, (hmr[0].start_hpa + (1ULL << 30)), &s->dmabuf_mem_region);
...
static void leap_ivshmem_map_host_addr_space(IVShmemState *s);
...
static void leap_write_doorbell_slow(IVShmemState *s, uint64_t val, hwaddr addr,
        unsigned size)
{
    size_t dbsz = sizeof(uint32_t);
    uint32_t *pdb = (uint32_t *)(s->shbuf + 8 * 1024 * 1024);
    bool is_sq;
    int idx;

    if ((addr % dbsz) || size != dbsz) {
        printf("Coperd,read " TARGET_FMT_plx ", size=%ld\n", addr, dbsz);
    }

    /* Coperd: ok, doorbell read for testing using fake NVMe */
    idx = (addr / dbsz) + 14; /* Coperd: no magic, -4 + 18 */
    is_sq = (idx % 2) ? false : true;

    pdb[idx] = (uint32_t)val;
    printf("Coperd,%s,write qid(%d)-%s:%d\n", __func__, idx / 2,
            is_sq ? "SQ" : "CQ", pdb[idx]);
}
                             ...
                             uint64_t val, unsigned size)
{
    ...
    switch (addr)
    {
        ...
        case HOST_MEM_MMAP:
            leap_ivshmem_map_host_addr_space(s);
            break;

        default:
            leap_write_doorbell_slow(s, val, addr, size);
#if 0
        default:
...
static int leap_read_doorbell_slow(IVShmemState *s, hwaddr addr, unsigned size)
{
    size_t dbsz = sizeof(uint32_t);
    uint32_t *pdb = (uint32_t *)(s->shbuf + 8 * 1024 * 1024);
    bool is_sq;
    ...
    printf("Coperd,%s,read qid(%d)-%s:%d\n", __func__, idx / 2,
            is_sq ? "SQ" : "CQ", pdb[idx]);
                                ...
                                unsigned size)
{
    ...
    switch (addr)
    {
        ...
        default:
            return leap_read_doorbell_slow(s, addr, size);
...
static void leap_test_doorbell_dmap(IVShmemState *s)
{
    uint32_t *dbs = (uint32_t *)(s->shbuf + 8 * 1024 * 1024);
    printf("Coperd,############db[9]=%"PRIu32"#################\n", dbs[18]);
    dbs[18] = 20;
    printf("Coperd,########################after set 20 to qid 9's DB\n");
}

/* Coperd: TODO TODO */
#if 0
static void get_host_max_hpa(void)
{
    // sudo grep -i "system ram" /proc/iomem | tail -n 1 | awk '{print $1' | awk
    // -F- '{print $1'])
}
#endif
...
static void leap_ivshmem_map_host_addr_space(IVShmemState *s)
{
    ...
    /* Coperd,IVSHMEM BAR4: 1-256GB -> host mem, assuming host DRAM <= 255GB */
    host_mem_sz = (1ULL << 37); // - (1ULL << 30);
    ...
    memory_region_init_ram_ptr(&s->host_mem_region, OBJECT(s),
            "leap-host-mem-region", host_mem_sz, host_mem_ptr);
    memory_region_set_fd(&s->host_mem_region, host_mem_fd);
    /* Coperd: IMPORTANT, skip VFIO DMA MAPPING for this MR */
    s->host_mem_region.should_skip = true;

    /* Coperd: the offset to be 1GB */
    memory_region_add_subregion(&s->bar3, 1ULL << 30, &s->host_mem_region);

    /*
     * Coperd: Disable hugepage mapping into IVSHMEM since it's mapped as part
     * of VM DRAM
     */
    if (0) {
        if (s->do_dmabuf_mapping) {
            /* do dmabuf init first */
            leap_init_dmabuf();
            /* then, we can store dmabuf mr info somewhere for socp usage */
            leap_map_dmabuf_mr(s);
        }
    }
}

static void leap_ivshmem_map_memfile(PCIDevice *dev)
{
    IVShmemState *s = IVSHMEM_COMMON(dev);
...
static void leap_ivshmem_map_qpairs(PCIDevice *dev)
{
    int ret;
    IVShmemState *s = IVSHMEM_COMMON(dev);
    unsigned long qpair_bar_sz = 16UL * 1024 * 1024;
    uint64_t pqp_db_addr;
    uint8_t attr = PCI_BASE_ADDRESS_SPACE_MEMORY | PCI_BASE_ADDRESS_MEM_PREFETCH;
    int r;

    attr |= PCI_BASE_ADDRESS_MEM_TYPE_64;

    /* Coperd: Leap maps vQP, vQP-DB, and pQP to vaddr: s->shbuf */
    if (posix_memalign(&s->shbuf, 4096, qpair_bar_sz)) {
        printf("Coperd,%s,cannot alloc 16MB 4KB aligned buffer for Leap\n", __func__);
        exit(EXIT_FAILURE);
    }
    assert((unsigned long)s->shbuf % 4096 == 0);
    memset(s->shbuf, 0, qpair_bar_sz);
    r = mlock(s->shbuf, qpair_bar_sz);
    if (r) {
        printf("Coperd,%s,mlock() failed\n", __func__);
        exit(EXIT_FAILURE);
    }

    memory_region_init_ram_ptr(&s->qpair_bar, OBJECT(s), "qpair-bar",
            qpair_bar_sz, s->shbuf);
    s->qpair_bar.should_skip = true;
    memory_region_add_subregion(&s->bar3, 0, &s->qpair_bar);

    ret = leap_do_wpt_ioctl_map((unsigned long)s->shbuf);
    if (ret) {
        printf("Coperd,%s,leap_do_wpt_ioctl_map failed\n", __func__);
        exit(EXIT_FAILURE);
    }
    /* Coperd: needed for buffer unmap from main() */
    soc_shared_qpair_buf = s->shbuf;

    /* Coperd: handle pQP-DB mapping separately */
    s->doorbell_bufsz = getpagesize();
    ret = leap_do_wpt_ioctl_get_pdb((unsigned  long)&pqp_db_addr);
    if (ret == -1) {
        printf("Coperd,%s,failed to get pQP DB page\n", __func__);
        exit(EXIT_FAILURE);
    }
    printf("Coperd,%s,pQP DB: %lx\n", __func__, pqp_db_addr);
    /* Coperd: 0xfebd1000, 0xdf211000 */
    s->doorbell_buf = leap_map_pqp_doorbell(pqp_db_addr, s->doorbell_bufsz);
    memory_region_init_ram_device_ptr(&s->pqp_doorbell, OBJECT(s),
            "pqp-doorbell", s->doorbell_bufsz, s->doorbell_buf);
    printf("Coperd,%s,register ivshmem BAR1 as NVMe-doorbell\n", __func__);
    pci_register_bar(dev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY, &s->pqp_doorbell);

    if (0)
        leap_test_doorbell_dmap(s);
}
...
static void ivshmem_common_realize(PCIDevice *dev, Error **errp)
{
    ...
    if (s->hostmem != NULL) {
        ...
        printf("Coperd,%s,to call host_memory_backend_get_memory\n", __func__);
        s->ivshmem_bar2 = host_memory_backend_get_memory(s->hostmem,
                &error_abort);
        s->ivshmem_bar2->should_skip = true;
    } else {
    ...
    const uint64_t bar_size = (16ULL << 20); /* 256GB */
    memory_region_init(&s->bar3, OBJECT(s), "leap-memfile", bar_size);
    printf("Coperd,%s,====ivshmem->bar3 inited=======\n", __func__);
    ...
    /* Coperd: map QuantumLeap related memory file & qpairs to SoC */
    if (0) {
        leap_ivshmem_map_memfile(dev);
    }

    /* Coperd: by default, don't map qpairs, so FEMU can be used without WPT */
    if (s->do_qpair_mapping) {
        leap_ivshmem_map_qpairs(dev);
    }
}
...
static void ivshmem_exit(PCIDevice *dev)
{
...
#if 0
            /* Coperd: do qp unmapping here */
            printf("Coperd,%s,ioctl(UNMAP) for safe teardown\n", __func__);
            ret = leap_do_wpt_ioctl_unmap((unsigned long)addr);
            if (ret == -1) {
                printf("Coperd,leap_do_wpt_ioctl_unmap failed\n");
                exit(EXIT_FAILURE);
            }
#endif
    ...
    leap_unmap_pqp_doorbell(s->doorbell_buf);
...
static Property ivshmem_plain_properties[] = {
    ...
    DEFINE_PROP_UINT8("do_qpair_mapping", IVShmemState, do_qpair_mapping, 0),
    DEFINE_PROP_UINT8("do_dmabuf_mapping", IVShmemState, do_dmabuf_mapping, 0),
    DEFINE_PROP_CHR("chardev", IVShmemState, chr),
    DEFINE_PROP_END_OF_LIST(),
};

static int leap_chr_can_read(void *opaque)
{
    return LEAP_HDR_SIZE;
}

/* Coperd: TODO TODO TODO */
static void leap_map_mem_table(IVShmemState *s)
{
    LeapMemory *memory = &s->memory; //.payload.memory;
    hwaddr subregion_offset = 1ULL << 30; // 1GB
    int *fds = s->fds;
    uint32_t i;

    if (memory->nregions > LEAP_MEMORY_MAX_NREGIONS) {
        printf("Coperd,%s,ERROR,nregions=%d > (MAX:%d)\n", __func__,
                memory->nregions, LEAP_MEMORY_MAX_NREGIONS);
        return;
    }

    /* Coperd: TODO: need to check offset alignment */

    for (i = 0; i < memory->nregions; i++) {
        LeapMemoryRegion *input = &memory->regions[i];
        LeapMemTableRegion *region = &s->mem_table[i];
        void *mmap_addr;
        ...
        /* Coperd: only map necessary memory file regions */
        mmap_addr = mmap(0, input->memory_size, PROT_READ | PROT_WRITE,
                         MAP_SHARED, fds[i], input->mmap_offset);
        close(fds[i]);
        fds[i] = -1;
        ...
        printf("\n\nCoperd,%s,region[%d],guest_phys_addr:0x%lx,memory_size:0x%lx,"
                "userspace_addr:0x%lx,mmap_offset:0x%lx,mmap_addr:0x%lx\n\n",
                __func__, i,
                memory->regions[i].guest_phys_addr,
                memory->regions[i].memory_size,
                memory->regions[i].userspace_addr,
                memory->regions[i].mmap_offset,
                (uint64_t)region->mmap_addr);

        memory_region_init_ram_ptr(&region->mr, OBJECT(s),
                "leap-mem-table-region",
                input->memory_size, region->mmap_addr);
        memory_region_add_subregion(&s->bar3, subregion_offset, &region->mr);
        printf("Coperd,%s,created a region to bar3,offset:0x%lx,size:0x%lx\n",
                __func__, subregion_offset, input->memory_size);
...
static void leap_chr_read(void *opaque, const uint8_t *buf, int size)
{
    IVShmemState *s = opaque;
    CharBackend *chr = &s->chr;
    LeapMsg *msg = &s->read_msg;
    uint8_t *p = (uint8_t *)msg;
    //int fd;

    if (size != LEAP_HDR_SIZE) {
        printf("Coperd,LEAP,Wrong message size received %d\n", size);
        return;
    }

    printf("Coperd,%s,copy %d bytes into s->read_msg\n", __func__, size);
    memcpy(p, buf, LEAP_HDR_SIZE);

    if (msg->size) {
        p += LEAP_HDR_SIZE;
        size = qemu_chr_fe_read_all(chr, p, msg->size);
        if (size != msg->size) {
            printf("Coperd,LEAP,Wrong message size received %d != %d\n",
                    size, msg->size);
            return;
        }
    }
    ...
    switch (msg->request) {
    case LEAP_SET_MEM_TABLE:
        /* received the mem table */
        memcpy(&s->memory, &msg->payload.memory, sizeof(msg->payload.memory));
        s->num_fds = qemu_chr_fe_get_msgfds(chr, s->fds, ARRAY_SIZE(s->fds));
        if (s->num_fds != s->memory.nregions) {
            printf("Coperd,%s,ERROR s->num_fds(%d) != s->memory.nregions(%d)\n",
                    __func__, s->num_fds, s->memory.nregions);
        }

        /* Coperd: TODO TODO: mmap QEMY memory file into IVSHMEM BAR3 */
        leap_map_mem_table(s);
        break;

    default:
        printf("Coperd,%s,only support SET_MEM_TABLE for now!!!!\n", __func__);
        break;
    }
}
...
static void ivshmem_plain_realize(PCIDevice *dev, Error **errp)
{
    ...
    if (!qemu_chr_fe_backend_connected(&s->chr)) {
        printf("Coperd,starting IVSHMEM without chardev");
    } else {
        qemu_chr_fe_set_handlers(&s->chr, leap_chr_can_read, leap_chr_read, NULL,
                s, NULL, true);
    }
