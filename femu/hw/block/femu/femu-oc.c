...
static int femu_ocadmin_passthru_getbbtbl(NvmeCmd *cmd, void *buf)
{
    int ret;
    struct wpt_admin_passthru_data pd;

    memcpy(&(pd.c), cmd, sizeof(NvmeCmd));
    pd.vm_pid = getpid();
    pd.buf = buf;

    ret = leap_do_wpt_ioctl_admin_passthru_getbbtbl(&pd);
    if (ret < 0) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }
...
uint16_t femu_oc_identity(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    /* Coperd: we passthru the `identity` command to OCSSD */
    ret = leap_do_wpt_ioctl_admin_passthru_identity(&n->femu_oc_ctrl.id_ctrl);
    if (ret < 0) {
        return NVME_INVALID_FIELD | NVME_DNR;
    }

    /* Coperd: for now, copy identity from OCSSD directly to VM */
    //return femu_ocadmin_passthru_identity(&n->femu_oc_ctrl.id_ctrl);
    //////////////////////////////////////////////
...
uint16_t femu_oc_bbt_get(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    int blks_per_lun = 1020 * 2; //c->num_blk * c->num_pln;
    int lun, ch, lunid;
    FEMU_OC_Bbt *bbt;
    ...
    ch = (ppa & ln->ppaf.ch_mask) >> ln->ppaf.ch_offset;
    lun = (ppa & ln->ppaf.lun_mask) >> ln->ppaf.lun_offset;
    lunid = ch * 8 + lun; //ch * c->num_lun + lun;
    printf("Coperd,%s,ch=%d,lun=%d,lunid=%d\n", __func__, ch, lun, lunid);
    bbt = ns->bbtbl[lunid];

    /*
     * Coperd: for leap, we do command passthrough here to retrieve info
     * directly from OCSSD controller
     */
    ret = femu_ocadmin_passthru_getbbtbl(cmd, bbt);
    if (ret) {
        printf("Coperd,%s,getbbtbl passthrough failed\n", __func__);
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ////////////////////////// END ////////////////////////////////////
    ///////////////////////////////////////////////////////////////////
...
int femu_oc_init_meta(FEMU_OC_Ctrl *ln)
{
    ...
    /* Coperd :Disable this for QuantumLeap */
#if 0
...
int femu_oc_bbtbl_init(NvmeCtrl *n, NvmeNamespace *ns)
{
    ...
    nr_tt_luns = 16 * 8; //c->num_ch * c->num_lun;
    blks_per_lun = 1020 * 2; //c->num_blk * c->num_pln;
...
static int femu_init_identity13(FEMU_OC_Ctrl *ln)
{
    FEMU_OC_IdCtrl13 *id13 = (FEMU_OC_IdCtrl13 *)&ln->id_ctrl;
    ...
    id13->ppaf.ch_offset = 25;
    id13->ppaf.ch_len = 4;
    id13->ppaf.lun_offset = 22;
    id13->ppaf.lun_len = 3;
    id13->ppaf.pln_offset = 2;
    id13->ppaf.pln_len = 1;
    id13->ppaf.blk_offset = 12;
    id13->ppaf.blk_len = 10;
    id13->ppaf.pg_offset = 3;
    id13->ppaf.pg_len = 9;
    id13->ppaf.sect_offset = 0;
    id13->ppaf.sect_len = 2;

    id13->mccap = 0;

    id13->geo.num_ch = 16;
    id13->geo.num_lun = 8;
    id13->geo.num_chnks = 1020;
    id13->geo.clba = 4096;
    id13->geo.csecs = 4096;
    id13->geo.sos = 16;
    ...
    id13->wrt.mw_min = 4;
    id13->wrt.mw_opt = 8;
    id13->wrt.mw_cunits = 0; //

    id13->perf.trdt = 40000;
    id13->perf.trdm = 80000;
    id13->perf.tprt = 1900000;
    id13->perf.tprm = 3700000;
    id13->perf.tbet = 7000000;
    id13->perf.tbem = 20000000;

    return 0;
}

/* Coperd: FEMU original calculatd parameters */
static int femu_init_params(FEMU_OC_Ctrl *ln)
{
    ln->params.pgs_per_blk = 512;

    /* calculated values */
    ln->params.sec_per_pl = 4 * 2/*ln->params.sec_per_pg * c->num_pln*/;
    ln->params.sec_per_blk = ln->params.sec_per_pl * 512 /*ln->params.pgs_per_blk*/;
    ln->params.sec_per_lun = ln->params.sec_per_blk * 1020/*c->num_blk*/;
    ln->params.sec_per_ch = ln->params.sec_per_lun * 8/*c->num_lun*/;
    ln->params.total_secs = ln->params.sec_per_ch * 16/*c->num_ch*/;
    printf("Coperd,sec_per_pl=%d,sec_per_blk=%d,sec_per_lun=%d,total_secs=%d\n",
            ln->params.sec_per_pl, ln->params.sec_per_blk,
            ln->params.sec_per_lun, ln->params.total_secs);

    /* Calculated unit values for ordering */
    ln->params.pl_units = ln->params.sec_per_pg;
    ln->params.pg_units = ln->params.pl_units * 2/*c->num_pln*/;
    ln->params.blk_units = ln->params.pg_units * ln->params.pgs_per_blk;
    ln->params.lun_units = ln->params.blk_units * 1020/*c->num_blk*/;
    ln->params.ch_units = ln->params.lun_units * 8/*c->num_lun*/;
    ln->params.total_units = ln->params.ch_units * 16/*c->num_ch*/;
    printf("Coperd,pl_units=%d,pg_units=%d,blk_units=%d,lun_units=%d,"
            "ch_units=%d,total_units=%d\n", ln->params.pl_units,
            ln->params.pg_units, ln->params.blk_units, ln->params.lun_units,
            ln->params.ch_units, ln->params.total_units);

    /* Coperd: don't need this one since we already retried info from OCSSD */
...
static int femu_oc_init_namespace(NvmeCtrl *n, int i)
{
    FEMU_OC_Ctrl *ln;
    NvmeNamespace *ns;
    FEMU_OC_IdGroup *c;
...
int femu_oc_init(NvmeCtrl *n)
{
    ...
    for (i = 0; i < n->num_namespaces; i++) {
        ns = &n->namespaces[i];
        /* Coperd: init vOC controller with CNEX OCSSD identify info */
        femu_init_identity13(ln);
        femu_init_params(ln);

        /* Coperd: we did this on purpose */
        if (0)
            femu_oc_init_namespace(n, i);
