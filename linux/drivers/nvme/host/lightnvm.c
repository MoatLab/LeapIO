		...
		struct nvme_nvm_id *nvme_nvm_id)
{
    ...
    struct nvme_nvm_addr_format *ppaf = &id->ppaf;
    int i;

	memcpy(&nvm_id->ppaf, &id->ppaf, sizeof(struct nvme_nvm_addr_format));

    pr_debug("Coperd,lbaf:\n");
    for (i = 0; i < 8; i++)
        pr_debug("Coperd,[%d]:%d\n", i, id->lbaf[i]);

    pr_debug("Coperd,ppaf:\n\n");
    pr_debug("ch_offset=%d,ch_len=%d\n", ppaf->ch_offset, ppaf->ch_len);
    pr_debug("lun_offset=%d,lun_len=%d\n", ppaf->lun_offset, ppaf->lun_len);
    pr_debug("pln_offset=%d,pln_len=%d\n", ppaf->pln_offset, ppaf->pln_len);
    pr_debug("blk_offset=%d,blk_len=%d\n", ppaf->blk_offset, ppaf->blk_len);
    pr_debug("pg_offset=%d,pg_len=%d\n", ppaf->pg_offset, ppaf->pg_len);
    pr_debug("sect_offset=%d,sect_len=%d\n\n", ppaf->sect_offset, ppaf->sect_len);
    ...
    pr_debug("Coperd,num_ch=%d,num_lun=%d,num_chnk=%d,clba=%d,csecs=%d,sos=%d\n",
            grp->num_ch, grp->num_lun, grp->num_chk, grp->clba, grp->csecs,
            grp->sos);
    ...
    pr_debug("Coperd,mw_opt=%d,mw_min=%d,mw_cunits=%d\n", grp->ws_opt,
            grp->ws_min, id->wrt.mw_cunits);
    ...
    pr_debug("Coperd,trdt:%d,trdm:%d,tprt:%d,tprm:%d,tbet:%d,tbem:%d\n",
            grp->trdt, grp->trdm, grp->tprt, grp->tprm, grp->tbet, grp->tbem);
    ...
    pr_debug("Coperd,mccap=%d\n", le32_to_cpu(id->mccap));
